class CallArgCases {
  ...
  void argsFirstBreakingChain(Builder builder, Config someConfiguration, User user, boolean flag) {
    builder.withValue(someConfiguration.getReader().getAuthorization(user).stream().filter(a -> !a.isEmpty()).toList(), flag);
  }
  ...
  void argsLastBreakingChain(Config config, Model userAuthorizationModel, User user) {
    target.accept(userAuthorizationModel, config.getUserAuthorizationApi().getAuthorization(user).stream().filter(a -> !a.isEmpty()).toList());
  }
  ...
  void test() {
    ...
    final Map<String, List<SomeVeryLongTypeName>> extremelyLongVariableNameForThisDeclaration = someFactory.createComplicatedInstance(firstArg, secondArg);
  }
