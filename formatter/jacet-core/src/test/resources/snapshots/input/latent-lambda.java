class LatentLambda {
  ...
  void argExpand(Cache cache, Config config, No no) {
    final List<Range> noRanges = cache.getRanges(no, config, () ->
      config.getGroupConfigForNestedNodeValue().getValueForChild(no)
    );
  }
  ...
  List<Item> argExtraIndent(List<Item> list, Service service) {
    return list.stream()
      .filter(item ->
        !service.isAggregatedValueOverMax(item.entity(), item.combinedValue().getAmount())
      )
      .toList();
  }
  ...
  void argHug(List<Item> list) {
    list.stream()
      .filter(
        Util.distinctByKey(item ->
          String.join(",", item.getA(), item.getB())
        )
      )
      .toList();
  }
  ...
  void argInlineBreak(List<Action> actions, List<Step> steps) {
    actions.removeIf(
      action -> (
        ...
        steps
          .stream()
          .noneMatch(s -> s.getType() == action.getType())
      )
    );
  }
  ...
  List<Range> bodyWrap(Parameter parameter) {
    return parameter
      .getRecords()
      .getSelectedMappedChildItem(IndexedRecordValueViewX::isPickedForGroup, p ->
        new Range(p.getStartIndexValueX(), p.getEndIndexValueX())
      );
  }
  ...
  void chainWrap(Service service) {
    assertThatThrownBy(() ->
      service.execute(param1, param2, param3, param4)
  ...
  Optional<BigDecimal> paramsInMethodCall(Dto dto) {
    return Optionals.mapIfAllPresent(
      dto.getFirstRecordValue(),
      dto.getSecondValue(),
      (firstRecord, external) -> firstRecord.getTallied().add(external)
    );
  }
  ...
  void methodArgVsChainLambda(Service someService) {
    when(someService.findByVeryLongMethodName(param))
      .thenAnswer(
        invocation -> {
