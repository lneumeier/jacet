class ChainCases {
  ...
  void forceExpand(Object result) {
    assertThat(result).usingRecursiveComparison().isEqualTo(new LoggerDto(Boolean.FALSE, Boolean.FALSE, Boolean.FALSE, Boolean.FALSE));
  }
  ...
  void forceWrapReceiver(Object mock) {
    await().atMost(5, SECONDS).untilAsserted(() -> verify(mock).handleSomething(expectedValue));
  }
  ...
  void chainInBinopArg() {
    nodeEntity = new NodeEntity(nodeService.findFirstByOrderByNodeNumberDesc().orElseGet(() -> new NodeEntity(0L)).getNodeNumber() + 1);
  }

  // Standalone chain in binop — same issue
  void standaloneChainBinop() {
    final long number = nodeService.findFirstByOrderByNodeNumberDesc().orElseGet(() -> new NodeEntity(0L)).getNodeNumber() + 1;
  }
  ...
  java.util.List<View> loadAll(NodeFilter nodeFilter, Sort recordSort) {
    return this.distinctProjectionRepository.findAll(GenericLayoutNode.class, specification, GenericNodeProjection.FIELD_KEYS, GenericNodeProjection.class, recordSort).stream().map(adapter::toView).toList();
  }
  ...
  java.util.stream.Stream<X> factoryHead() {
    return com.example.Stream.of(firstElementValueHere, secondElementValueHere, thirdElementValueHere).filterElements(predicate).mapElements(mapper);
  }
  ...
  public void thisIsFactory() {
    this.getNode(nodeId).map(node -> updateValue(node)).orElse(false);
    this.getNode(nodeId).map(node -> node.updateValue()).filter(x -> x.isValid()).orElse(false);
  }
  ...
  void assignmentChainRhs(ContainerNodeRelations containerNodeRelationGroup, int i) {
    final GenericExternalSourceFetchNodeChildRelationsRowsItem childNodeRelationItems = containerNodeRelationGroup.getChildRelationSet().get(i);
  }

  void chained(SequenceNodeRepository sequenceNodeRepository, GroupingId groupingId, String sequenceType) {
    final SequenceNodeEntity sequenceNode = sequenceNodeRepository.findById(new SequenceNodeEntityId(groupingId, sequenceType)).orElseGet(() -> this.createSequenceNode(groupingId, sequenceType));
  }
