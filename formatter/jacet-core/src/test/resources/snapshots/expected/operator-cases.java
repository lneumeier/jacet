class OperatorCases {
  ...
  void binopChainPlainAssign(ItemSource itemSource) {
    nextNumber =
      itemSource
        .findFirstByOrderByNodeIndexDesc()
        .map(ValueNode::getAmount)
        .orElseGet(() -> 0L) + 1;
  }
  ...
  boolean isSameNodeResultStatus(Result result, int processRecordIndex, ValueContainer error) {
    return (
      this.isSameNodeValue(result, error) &&
      result
        .getProcessRecordIndex()
        .map(index -> index.compareTo(processRecordIndex))
        .orElse(-1) == 0
    );
  }
  ...
  private boolean returnInParens(final Container container) {
    return (
      container.getTotalAmount().signum() != 0 ||
      container.getSubsetAmount().signum() != 0 ||
      container.getNestedEntryWeightTotal().signum() != 0
    );
  }
  ...
  public void localAssign() {
    final ALongClassNameThatTriggersBreaking adapter = new ALongClassNameThatTriggersBreaking(
      firstArg,
      secondArg,
      thirdArg,
      fourthArg,
