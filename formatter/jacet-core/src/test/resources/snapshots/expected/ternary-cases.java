class TernaryCases {
  ...
  void parenTernary() {
    final String value = (isConditionMet
      ? someVeryLongValueThatNeedsWrappingBecauseItExceedsTheLineWidth
      : anotherVeryLongAlternativeValueThatAlsoExceeds);
  }

  // Ternary in parens used as method argument
  void parenTernaryAsArg() {
    process(
      (isConditionMet ? someVeryLongValueThatNeedsWrappingBecauseItExceedsTheLineWidth : anotherVeryLongAlternativeValueThatAlsoExceeds)
    );
  }

  // Ternary in parens with chain — (cond ? a : b).method()
  void parenTernaryWithChain() {
    final String result = (isConditionMet ? getFirstOptionValueFromConfiguration(config) : getSecondOptionValueFromConfiguration(config))
      .toString()
      .trim();
  }
  ...
  String compute(int a, int b, int c, int d) {
    return a > b && c > d && someOtherConditionThatMakesThisLineExceedOneHundredAndFortyCharactersInWidth
      ? "first branch value result"
      : "second branch value result";
  }

  // Throw with binary condition ternary
  void validate(int value) {
    throw value > 0 && value < maxThreshold && isValidationEnabled
      ? new IllegalArgumentException("Value " + value + " out of range for the configured maximum threshold")
      : new IllegalStateException("Validation is disabled but value check was triggered unexpectedly");
  }
  ...
  String getValue(NodeId nodeId, GroupId groupId, ConfigType configType) {
    return configType == ConfigType.PRIMARY
      ? nodeConfiguration.getPrimaryNodeValue(nodeId)
      : groupService
          .getGroup(groupId)
          .map(GroupEntity::getChildNodeValue)
          .orElseThrow(() -> new InvalidInputException(ErrorCode.GROUPS_NOT_FOUND));
  }
  ...
  void longBinaryCondition() {
    final String longVariableName =
      someConditionPartOne > 0 && someConditionPartTwoThatIsAlsoVeryLongAndMakesTheWholeThingExceedTheLineWidthLimit
  ...
  String computeLong() {
    final String result = veryLongCondition.method().method().anotherCallIsXYZ().getOtherXYZAB()
      ? firstValueLongNameXX
      : secondValueShorter;
    return result;
  }
  ...
  String deepClassify(int value) {
    final String result =
      value > 1000
        ? computeVeryHighCategory(value, options)
        : value > 500
          ? computeHighCategory(value, options)
          : value > 100
            ? computeMediumCategory(value, options)
            : computeLowCategory(value, options);
    return result;
  }
