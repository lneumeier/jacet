class LambdaBodyCases {
  ...
  Predicate<Data> binopIndent() {
    return data ->
      SomeVeryLongClassName.isConditionAlphaExtended(data) ||
      SomeVeryLongClassName.isConditionBravoExtended(data) ||
      SomeVeryLongClassName.isConditionCharlieExtended(data);
  }
  ...
  Predicate<Filter> longOrChain() {
    return filter ->
      ("field2".equals(filter.getField()) && "2".equals(filter.getValue())) ||
      ("field3".equals(filter.getField()) && "x".equals(filter.getValue())) ||
      ("field4".equals(filter.getField()) && "y".equals(filter.getValue()));
  }
  ...
  void binopBody() {
    BaseEntity.unproxyIf(
      this.selectedEntity,
      SelectedEntityBaseObject.class,
      () -> StringUtils.isNotBlank(this.recordNumber) && this.selectedEntityNumber > 0
    );
  }

  // hang-expand-rules: lambda body is MethodInvocation — expandable. Hang.
  void methodInvocationBody() {
    list.computeIfAbsent(longKeyExpression, longSecondArgExpression, key ->
      repository.lookupByKeyAndExtraArgumentAndAnotherToForceWrapping(key)
    );
  }

  // hang-expand-rules: lambda body is Ternary — expandable. Hang.
  void ternaryBody() {
    list.process(longFirstArg, secondLongArgument, value ->
      value > 0 ? handlerForPositiveValuesOnly(value) : handlerForZeroOrNegative(value)
    );
  }
  ...
  void parenBody(String value) {
    Assert.isTrue(StringUtils.isNumeric(value) || SUPPLEMENTARY_PERIOD_DOMAIN_VALUE.equalsIgnoreCase(value), () ->
      ("The period may only consist of numeric characters or " + SUPPLEMENTARY_PERIOD_DOMAIN_VALUE + " here.")
    );
  }
  ...
  void unaryBody() {
    return loginHistory
      .stream()
      .filter(userLoginHistoryEntity ->
        !(userLoginHistoryEntity.isSuccessful() && !MAX_FAILED_ATTEMPTS_MESSAGE.equals(userLoginHistoryEntity.getLoginReason()))
      )
      .toList();
  }
  ...
  void assignmentBody() {
    final Advice modelInterceptor = new ModelProxyInterceptor("", (fieldPath, _, _) ->
      callMapping.modelPath = this.getPathWithoutDomainTypeValueAndSomeMorePaddingHere(fieldPath)
    );
  }
  ...
  void binaryBodyStaysExpanded(String first, String second) {
    someMethodWithAVeryLongNameHere(
      firstArgumentValueGoesHereAndIsLong,
      () -> firstOperandValueIsAlsoQuiteLong + secondOperandValueHereToo
    );
  }
