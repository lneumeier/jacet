class LambdaArgCases {
  ...
  Object hangBodyWhenOverflow(java.util.Optional<Config> dbConfiguration, Config config) {
    if (flag) {
      return dbConfiguration.stream().filter(c -> c.matches()).findFirst().orElseThrow(() -> new AppRuntimeException(String.format("Configuration entry '%s' could not be located!", config.getOptionName())));
    }
    return null;
  }
  ...
  Set<Change> shortBodyExpandsAllArgs(ValidateMetricRows validateMetricRows) {
    return Optionals.mapIfAllPresent(validateMetricRows.getMetricRowsOriginal(), validateMetricRows.getMetricRowsUpdated(), (original, updated) -> Set.of(of(A, original.getA(), updated.getA()), of(B, original.getB(), updated.getB())));
  }
  ...
  Set<Change> breakingBodyHangsUnderArrow(ValidateMetricRows validateMetricRows) {
    return Optionals.mapIfAllPresent(validateMetricRows.getMetricRowsOriginal(), validateMetricRows.getMetricRowsUpdated(), (original, updated) -> Set.of(of(ROW_SPAN_TOTAL_1, original.getRowSpanTotal1(), updated.getRowSpanTotal1(), validateMetricRows, allowedFunc), // hint
  ...
  void hangingLongPrecedingArgs(final Service service) {
    service.runAndNotifyEverybodyAboutEventVeryThoroughly("first-very-long-arg-here", "second-very-long-arg-here", "third-very-long-arg-here", () -> {
      doSomething();
      doMore();
    });
  }
  ...
  void middleLambdaBreaks() {
    return batchService.runAndNotify(directories.getImportFileList(), (final File file) -> ErrorHandling.run(() -> innerImportBatchService.runBatchStep(directories, file, userId), (throwable, errorCode, errorValue) -> NestedProcessingBatchResult.importError(file, errorCode, errorValue)), new BatchServiceNotificationParameter(batchId, "import"), _ -> this.deleteProcessedDirectories(directories));
  }

  // middle-trivial-lambda-fits: a short middle lambda stays in hanging layout
  void trivialMiddleLambda() {
    return service.process(items, x -> x.getId(), "default", result -> handleLongResultProcessingWithMultipleSteps(result, options, context));
  }

  // multi-lambda: two block lambdas as non-adjacent args
  void multiLambda(final Service service) {
    service.runAndNotify(arg1, () -> {
      doFirstLambdaWork();
    }, arg3, () -> {
      doSecondLambdaWork();
    });
