...
class Outer {
  private record NestedTreeParameter(@RecordNumber String recordNumber, @Note String note, boolean showAggregatedItemValues) implements RecordPartId {
    @Override
  ...
  private record NoInterface(@RecordNumber String recordNumber, @Note String note, boolean showAggregatedItemValues, String extraComponent) {}
}
...
class RecordPatternCases {
  ...
  long computeChainBody(Object obj) {
    return switch (obj) {
      case BigDecimal bd -> bd.subtract(baseline).movePointRight(precisionValue).setScale(0, RoundingMode.HALF_UP).longValueExact() + offset;
      default -> 0L;
    };
  }
}
...
class CompactConstructorComments {
  ...
  record Validated(int value) {
    ...
    public Validated { // trailing comment
      if (value < 0) {
