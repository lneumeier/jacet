...
@Config(
  name = "myService",
  description = "A very long description that explains what this service does in great detail",
  priority = 42,
  tags = { "api", "internal", "experimental", "deprecated" },
  handler = Void.class
)
...
@Complex(
  ...
  types = { String.class, Integer.class, Long.class, Double.class, Float.class },
  retention = @Retention(RetentionPolicy.RUNTIME),
  target = @Target({ ElementType.TYPE, ElementType.METHOD, ElementType.FIELD, ElementType.PARAMETER })
)
class KitchenSink {}
...
@ComponentScan(
  ...
  excludeFilters = { @ComponentScan.Filter(type = FilterType.REGEX, pattern = { "com.example.foo.first", "com.example.foo.second" }) }
)
