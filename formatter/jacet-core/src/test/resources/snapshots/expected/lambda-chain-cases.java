class LambdaChainCases {
  ...
  Optional<UserDto> updatePassword(final String name, final String pw) {
    return this.findByLoginName(name)
      .map(user -> {
        user.setLastName("");
        user.setPassword(pw);
  ...
  DocumentType lookup(Document document) {
    return document
      .getDocumentType()
      .orElseThrow(() -> new DocumentException("Unknown document type for input file: " + document.getFilename()));
  }
  ...
  java.util.List<Ou> chainBodyArgHangs(java.util.List<Ou> listToBeFiltered, java.util.List<Direct> directOrganizationUnitMembers) {
    return listToBeFiltered
      .stream()
      .filter(ou ->
        directOrganizationUnitMembers
          .stream()
          .map(OrganizationUnitBaseDataModel::getId)
          .anyMatch(id -> ou.getMainOrganizationUnit().getId().equals(id))
      )
      .toList();
  }
  ...
  void chainBodyOnlyArgHangs(java.util.List<Direct> directOrganizationUnitMembers) {
    doSomethingWithThisLongMethodName(ou ->
      directOrganizationUnitMembers
        .stream()
        .map(Foo::getId)
        .anyMatch(id -> ou.getOrg().getId().equals(id))
    );
  }
}
